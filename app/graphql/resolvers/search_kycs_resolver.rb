...
module Resolvers
  class SearchKycsResolver < Types::Base::BaseResolver
    ...
    argument :status, Types::Kyc::KycStatusEnum,
             required: false,
             default_value: nil,
             description: 'Filter KYCs by their status'
    argument :sort, Types::Kyc::SearchKycFieldEnum,
             required: false,
    ...
    def resolve(page: 1, page_size: 30, status: nil, sort: 'updated_at', sort_by: 'desc')
      ...
      entities = items.map { |kyc| KycTypes::KycEntity.from_model(kyc) }
      connection_class = GraphQL::Relay::BaseConnection.connection_for_nodes(entities)
    ...
    def self.authorized?(object, context)
      super &&
        ...
        Ability.new(user).can?(:read, KycTypes::KycEntity)
    end
  end
end
